...
const Cart = ({ match }) => {
  ...
  const [localCartState, setLocalCartState] = useState(cartItems);
  ...
  cart.itemsPrice = addDecimals(
    localCartState.reduce((acc, item) => acc + item.price * Number(item.qty), 0)
  );
        ...
        <Container fluid className=" mt-4 p-3 p-lg-3">
          <Row className="d-flex justify-content-around ">
            <Col lg={8} className="bg-white rounded py-3">
              ...
              {localCartState.map((cartItem) => {
                return (
                  <CartItem
                    key={cartItem.product}
                    changeCartState={setLocalCartState}
                    item={cartItem}
                    prevState={cartItems}
                  />
                );
